...
class Cons[+A](hd: A, t1: => MyStream[A]) extends MyStream[A] {
  ...
  def map[B](f: A => B): MyStream[B] = {
    new Cons(f(head), tail.map(f)) // preserves lazy eval
  }
  def flatMap[B](f: A => MyStream[B]): MyStream[B] = {
...
val naturals = MyStream.from(1)(_ + 1)
  ...
  startFrom0.map(_*2).take(100).toList().foreach(println)
  startFrom0.flatMap(x => new Cons(x, new Cons(x + 1, EmptyStream))).take(10).toList().foreach(println)
