...
//  def &(anotherSet: MySet[A]): MySet[A] =  filter(x => anotherSet.contains(x)) // same as
//  def &(anotherSet: MySet[A]): MySet[A] =  filter(x => anotherSet(x)) // same as
  def &(anotherSet: MySet[A]): MySet[A] =  filter(anotherSet) // same as
...
object MySetTest extends App {
  ...
  s foreach println
  s map (_ + 4) foreach println
  s flatMap (x => MySet(x, 10 * x)) foreach println
  s filter(x => x % 5 == 0) foreach println

  val s2 = MySet(1,2,3,4)
  println("s2")
  s2 - 1 foreach println
  println("remove 3 and 4")
  s2 -- MySet(3,4) foreach println
