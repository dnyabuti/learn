...
import com.nyabuti.files.{Directory, File}
...
class Echo(args: Array[String]) extends Command {
  override def apply(state: State): State = {
    ...
    // if > echo to file. create new file if none exists
    // if >> append to file
  ...
  def getRootAfterEcho(currentDirectory: Directory, path: List[String], contents: String, append: Boolean): Directory = {
    if (path.isEmpty) currentDirectory
      ...
      val newNextDirectory = getRootAfterEcho(nextDirectory, path.tail, contents, append = append)
  ...
  def doEcho(state: State, contents: String, filename: String, append: Boolean): State = {
    if (filename.contains(Directory.SEPARATOR))
      state.setMessage(s"Invalid filename. Filename must not contain ${Directory.SEPARATOR}")
    else {
      // create new root
