...
object DeployingActorsRemotely_LocalApp  extends App{
  ...
  println(simpleActor) //akka://RemoteActorSystem@localhost:2552/remote/akka/LocalActorSystem@localhost:2551/user/remoteActor
  ...
  class ParentActor extends Actor with ActorLogging {
    override def receive: Receive = {
      ...
      case Terminated(actorRef) =>
        log.info(s"Child actor $actorRef terminated")
    }
  }
