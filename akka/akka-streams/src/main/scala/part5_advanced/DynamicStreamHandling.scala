...
object DynamicStreamHandling extends App {
  ...
  // 1. Kill Switch (Dynamically stop a stream
  ...
  // This is a publisher subscriber component
  ...
  subscriberPort.map(string => string.length).runWith(Sink.foreach(n => println(s"I received a number: $n")))

  Source(List("Non", "-", "scratching", "microfiber")).runWith(publisherPort)
  Source(List("I", "like", "Scala")).runWith(publisherPort)
  Source.single("Data").runWith(publisherPort)
}
