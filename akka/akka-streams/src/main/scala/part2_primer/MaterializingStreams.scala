...
object MaterializingStreams extends App {
  ...
  val sink = Sink.reduce[Int]((a, b) => a + b) // Sink[Int, Future[Int]]
  ...
  graph.run().onComplete {
    ...
    case Failure(ex) => println(s"Stream processing failed with exception: $ex")
  }
  ...
  // return last element out o a source
  ...
  val sentenceSource = Source(List(
    ...
    "materialized values are interesting"
  ))
